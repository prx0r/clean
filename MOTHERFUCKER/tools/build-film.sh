...
cd "$ROOT"
mkdir -p build/audio-scenes build/logicvid-reality-appears
...
ffmpeg -y -loglevel error -f concat -safe 0 -i build/audio-scenes/concat.txt \
  -c:a libmp3lame -q:a 2 build/logicvid-reality-appears/narration.mp3

python3 tools/analyse-audio.py \
  build/logicvid-reality-appears/narration.mp3 \
  build/logicvid-reality-appears/features.json

node tools/render-logicvid.mjs

ffmpeg -y -loglevel error \
  -i build/logicvid-reality-appears/video.mp4 \
  -i build/logicvid-reality-appears/narration.mp3 \
  -c:v copy -c:a aac -b:a 192k \
  -map 0:v:0 -map 1:a:0 -shortest \
  build/logicvid-reality-appears/final.mp4

echo "Built build/logicvid-reality-appears/final.mp4"
