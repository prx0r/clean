#!/usr/bin/env python3
"""Trajectory engine shared by the phononic film, score and validators."""
...
def interpolation_amount(
    ...
    """Meaning discloses late when fragmented and flows when continuity is high."""
    continuity = 0.5 * (float(left["state"][1]) + float(right["state"][1]))
    fluid = smoothstep01(local)
    disclosure = smoothstep01(clamp01((local - 0.48) / 0.30))
    return mix(disclosure, fluid, continuity)
...
def beat_position_at(composition: dict[str, Any], seconds: float) -> float:
    """Numerically integrate the smooth tempo curve for deterministic visual pulse."""
...
def audio_features(
    ...
    if in_silence(composition, seconds):
        return 0.0, 0.0
    articulation, continuity, deixis, resonance, prosody, semantic_density = vector
    phase = beat_position - math.floor(beat_position)
    distance = min(phase, 1.0 - phase)
    width = mix(0.25, 0.055, prosody)
    beat = math.exp(-0.5 * (distance / width) ** 2)
    breath = 0.83 + 0.17 * math.sin(
        seconds * 0.13 + semantic_density * 4.0 + continuity
    )
    volume = (
        0.10
        + 0.20 * articulation
