#!/usr/bin/env python3
"""Render the contrapuntal forest with procedural orchestral instruments.
...
Examples:
    ...
    python render-audio.py --start 536 --end 552 --output build/recognition.wav
"""
...
def instrument(
    ...
    if style == 0:
        # Earth cello: bowed body, rosin edge, and a slow woody formant.
        bow = 0.035 * np.sin(absolute_time * 181.0 + seed)
        signal = harmonic(phase + bow, 1.0, 0.68, nyquist, frequency)
        signal += harmonic(phase, 2.0, 0.24, nyquist, frequency, 0.18)
        signal += harmonic(phase, 3.0, 0.12, nyquist, frequency, 0.62)
        signal += harmonic(phase, 5.0, 0.045, nyquist, frequency, seed)
        envelope = soft_envelope(age, remaining, 0.085, 0.22)
    elif style == 1:
        # Root viola: darker bowed countervoice with a gentle beating pair.
        signal = 0.43 * np.sin(phase * 0.998 + seed * 0.2)
        signal += 0.43 * np.sin(phase * 1.002 - seed * 0.1)
        signal += harmonic(phase, 2.0, 0.20, nyquist, frequency, 0.5)
        signal += harmonic(phase, 4.0, 0.07, nyquist, frequency, 1.2)
        envelope = soft_envelope(age, remaining, 0.070, 0.20)
    elif style == 2:
        # Leaf violin: brilliant upper string that remains soft at the attack.
        vibrato = 0.045 * np.sin(absolute_time * 5.2 + seed)
        signal = np.sin(phase + vibrato)
        signal += harmonic(phase, 2.0, 0.25, nyquist, frequency, 0.35)
        signal += harmonic(phase, 3.0, 0.14, nyquist, frequency, 0.9)
        signal += harmonic(phase, 6.0, 0.05, nyquist, frequency, 1.5)
        envelope = soft_envelope(age, remaining, 0.055, 0.16)
    elif style == 3:
        # Wind flute: breath-diffused fundamental with few upper partials.
        air = (
            np.sin(absolute_time * 137.0 + seed)
            * np.sin(absolute_time * 223.0 + 0.7)
        )
        signal = 0.79 * np.sin(phase + 0.035 * np.sin(absolute_time * 4.7))
        signal += harmonic(phase, 2.0, 0.13, nyquist, frequency, 0.4)
        signal += harmonic(phase, 3.0, 0.055, nyquist, frequency, 1.1)
        signal += air * 0.018
        envelope = soft_envelope(age, remaining, 0.075, 0.18)
    elif style == 4:
        # Light oboe: a focused odd-harmonic reed able to carry the subject.
        signal = harmonic(phase, 1.0, 0.58, nyquist, frequency)
        signal += harmonic(phase, 3.0, 0.25, nyquist, frequency, 0.18)
        signal += harmonic(phase, 5.0, 0.12, nyquist, frequency, 0.48)
        signal += harmonic(phase, 7.0, 0.055, nyquist, frequency, 0.92)
        envelope = soft_envelope(age, remaining, 0.045, 0.14)
    else:
        # River harpsichord: plucked harmonic current, deliberately without tail.
        ...
        signal *= np.exp(-age * 3.1)
        envelope = soft_envelope(age, remaining, 0.0025, 0.025)
...
def synthesize(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="forest-audio-") as temporary:
        mix_path = Path(temporary) / "forest-mix.f32"
        mix = np.memmap(
            mix_path,
            dtype=np.float32,
            mode="w+",
        ...
        for number, note in enumerate(selected):
            ...
            amplitude = (
                ...
                * note.volume
                * (0.060 if note.track != 5 else 0.042)
            )
...
def main() -> int:
    ...
    if args.output.suffix.lower() == ".wav":
        ...
        with tempfile.TemporaryDirectory(prefix="forest-encode-") as temporary:
