...
def build_narrated_storyboard(mod, output_dir: Path, args):
    ...
    for idx, scene in enumerate(scenes, 1):
        ...
        # Build slideshow: create a video from stills using concat demuxer
        stills_concat = scene_dir / "stills_concat.txt"
        seg_dur = dur / len(stills)
        lines = [f"file '{s.resolve()}'" for s in stills]
        stills_concat.write_text("\n".join(lines))

        # First create slideshow video from images
        slideshow_raw = scene_dir / "slideshow_raw.mp4"
        subprocess.run(
            [ffmpeg, "-y", "-f", "image2", "-framerate", f"{1/seg_dur:.2f}",
             "-i", str(scene_dir / "still_%02d.jpg"),
             "-c:v", "libx264", "-preset", "fast", "-crf", "23",
             "-pix_fmt", "yuv420p", str(slideshow_raw)],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )

        # Then mux with narration audio
        has_audio = audio_path.exists()
        cmd = [ffmpeg, "-y", "-i", str(slideshow_raw)]
        if has_audio:
            cmd.extend(["-i", str(audio_path),
                        "-c:v", "libx264", "-preset", "fast", "-crf", "23",
                        "-c:a", "aac", "-b:a", "192k", "-shortest"])
        else:
            cmd.extend(["-c:v", "libx264", "-preset", "fast", "-crf", "23", "-an"])
