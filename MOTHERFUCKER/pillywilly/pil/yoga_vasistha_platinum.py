#!/usr/bin/env python3
"""
THE YOGA VASISTHA AND PARALLEL UNIVERSES
The Story of Queen Lila — Ancient India's Multiverse

An original Platinum-house procedural visual essay.

CENTRAL CLAIM
-------------
The Yoga Vasistha, a 10th-century Sanskrit text, contains a story
strikingly parallel to modern multiverse theory: Queen Lila creates
an entire universe through the power of her mind, and lives multiple
simultaneous lives within it.

The text's doctrine of drishti-srishti-vada (perception-creation)
holds that the world is not independent of the perceiver — each act
of perception creates the world that is perceived. This is not
idealism in the Western sense. It is a sophisticated analysis of
the relation between consciousness, perception, and reality.

This does not mean the physical world is unreal.
It means the physical world is not independent of consciousness.

FILM THESIS
-----------
The modern picture:
world exists → consciousness perceives it

The Yoga Vasistha picture:
consciousness → perception → world appears

Modern physics (observer effect, participatory universe, quantum
measurement) is converging on a similar insight.

OUTPUT
------
output_yoga_vasistha/
"""
...
OUTPUT=Path("/mnt/HC_Volume_106427611/goldrender/output_yoga_vasistha")
...
def vis_v0(im,u,t,p):
    ...
    seal(im,"The Yoga Vasistha","A vast philosophical text disguised as a story.")
...
def vis_v1(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Queen Lila","A queen who creates an entire universe from her mind.")
...
def vis_v2(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"The parallel","Lila lives multiple simultaneous lives — each real.")
...
def vis_v3(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Drishti-srishti","Perception-creation: the world is not independent of the seer.")
...
def vis_v4(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Bhusunda the crow","A crow who witnesses the rise and fall of aeons.")
...
def vis_v5(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"The seven stages","Seven stages of wisdom from ignorance to liberation.")
...
def vis_v6(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Vairagya","Dispassion: the first step beyond suffering.")
...
def vis_v7(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Jivanmukta","Liberated while living — the goal of the path.")
...
def vis_v8(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Modern multiverse","The many-worlds interpretation meets ancient wisdom.")
...
def vis_v9(im,u,t,p):
    ...
    for j in range(int(8*r)):
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Observer effect","Quantum physics: the observer participates in creating reality.")

def vis_v10(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.50,h*.40; r=ease(u)
    for j in range(int(8*r)):
        a=j*math.tau/8+t*0.2; rr=25+15*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Caution","Parallel texts are not parallel proofs.")

def vis_v11(im,u,t,p):
    w,h=im.size; d=ImageDraw.Draw(im); cx,cy=w*.50,h*.40; r=ease(u)
    for j in range(int(8*r)):
        a=j*math.tau/8+t*0.2; rr=25+15*math.sin(t*0.3+j)
        x=cx+math.cos(a)*rr; y=cy+math.sin(a)*rr*.5
        ...
        d.ellipse((x-4,y-4,x+4,y+4),fill=(*PALE_GOLD,int(150*r)),outline=(*GOLD,int(120*r)),width=2)
    glow_circle(im,cx,cy,14,GOLD,int(150*r),11)
    seal(im,"Closing","Lila dreamed a universe. You are dreaming one now.")

@dataclass
class Scene:
    title:str; narration:str; duration:float; visual:str; params:dict
VISUALS = { "vis_v0": vis_v0, "vis_v1": vis_v1, "vis_v2": vis_v2, "vis_v3": vis_v3, "vis_v4": vis_v4, "vis_v5": vis_v5, "vis_v6": vis_v6, "vis_v7": vis_v7, "vis_v8": vis_v8, "vis_v9": vis_v9, "vis_v10": vis_v10, "vis_v11": vis_v11 }
SCENES = [ Scene("The Yoga Vasistha","A vast philosophical text disguised as a story.",7.0,"vis_v0",{}), Scene("Queen Lila","A queen who creates an entire universe from her mind.",7.0,"vis_v1",{}), Scene("The parallel","Lila lives multiple simultaneous lives — each real.",7.0,"vis_v2",{}), Scene("Drishti-srishti","Perception-creation: the world is not independent of the seer.",7.0,"vis_v3",{}), Scene("Bhusunda the crow","A crow who witnesses the rise and fall of aeons.",7.0,"vis_v4",{}), Scene("The seven stages","Seven stages of wisdom from ignorance to liberation.",7.0,"vis_v5",{}), Scene("Vairagya","Dispassion: the first step beyond suffering.",7.0,"vis_v6",{}), Scene("Jivanmukta","Liberated while living — the goal of the path.",7.0,"vis_v7",{}), Scene("Modern multiverse","The many-worlds interpretation meets ancient wisdom.",7.0,"vis_v8",{}), Scene("Observer effect","Quantum physics: the observer participates in creating reality.",7.0,"vis_v9",{}), Scene("Caution","Parallel texts are not parallel proofs.",7.0,"vis_v10",{}), Scene("Closing","Lila dreamed a universe. You are dreaming one now.",7.0,"vis_v11",{}) ]
...
def concat(paths):
    ...
    final=OUTPUT/"yoga_vasistha.mp4"
...
def export_timeline():
    ...
    p.write_text(json.dumps({"title":"the yoga vasistha and parallel universes","scene_count":len(SCENES),"runtime_seconds":round(cursor,3),"shot_duration_range":[5,10],"scenes":recs},indent=2,ensure_ascii=False),encoding="utf-8"); return p
